...
public class FuelCellTests {
  ...
  // Manifold capacity in mix-L (LH₂ + LOx combined). 1.466 mix-L
  // matches the production-design 33-min runtime: 1.466 / (4.96e-4
  // + 2.31e-4) ≈ 2017 s. Refill rate scales the same way (0.1466
  // mix-L/s). The cfg files use these same numbers.
  private const double SmallManifoldCap = 1.466;
  private const double SmallRefillRate  = 0.1466;

  private static FuelCell MakeFuelCell(
      bool isActive = false,
      double ec = SmallEcOutput,
      double manifold = SmallManifoldCap,   // start full unless overridden
      bool refillActive = false) {
    return new FuelCell {
      Lh2Rate = SmallLh2Rate,
      LoxRate = SmallLoxRate,
      EcOutput = ec,
      RefillRate = SmallRefillRate,
      Manifold = new Accumulator { Capacity = SmallManifoldCap, Contents = manifold },
      IsActive = isActive,
  ...
  public void Clone_CopiesAllState() {
    var src = new FuelCell {
      ...
      RefillRate = 0.2,
      Manifold = new Accumulator { Capacity = 3.0, Contents = 2.4 },
    ...
    Assert.AreEqual(3.0, dst.Manifold.Capacity);
    Assert.AreEqual(0.2, dst.RefillRate);
    Assert.AreEqual(2.4, dst.Manifold.Contents);
  ...
  public void State_RoundTripsAllFields() {
    var src = new FuelCell {
      IsActive = true, RefillActive = true,
      Manifold = new Accumulator { Capacity = 1.5, Contents = 0.42 },
    };
    ...
    var dst = new FuelCell {
      ...
      Manifold = new Accumulator { Capacity = 1.5 },
    };
    dst.Load(state);
    Assert.IsTrue(dst.IsActive);
    Assert.IsTrue(dst.RefillActive);
    Assert.AreEqual(0.42, dst.Manifold.Contents);
  }

  [TestMethod]
  public void Proportions_DerivedFromRates() {
    var fc = MakeFuelCell();
    var sum = SmallLh2Rate + SmallLoxRate;
    Assert.AreEqual(SmallLh2Rate / sum, fc.Lh2Frac, 1e-9);
    Assert.AreEqual(SmallLoxRate / sum, fc.LoxFrac, 1e-9);
    Assert.AreEqual(1.0, fc.Lh2Frac + fc.LoxFrac, 1e-9);
    Assert.AreEqual(sum, fc.ProductionDrainRate, 1e-9);
  }
  ...
  public void Refill_TurnsOnWhenManifoldLow() {
    var fc = MakeFuelCell(isActive: false, refillActive: false,
        manifold: 0.05 * SmallManifoldCap);   // 5%
  ...
  public void Refill_TurnsOffWhenManifoldFull() {
    ...
    Assert.IsFalse(fc.RefillActive, "Manifold at capacity should flip refill OFF");
  }
  ...
  public void Manifold_DrainsWhileProducing() {
    ...
    vessel.Tick(0.001);
    double initial = fc.Manifold.Contents;
    vessel.Tick(60.001);

    Assert.IsTrue(fc.Manifold.Contents < initial,
        $"Expected manifold drain, got {initial} → {fc.Manifold.Contents}");
  }
  ...
  public void Manifold_RefillsWhenLow() {
    ...
    var fc = MakeFuelCell(isActive: false, refillActive: false,
        manifold: 0.05 * SmallManifoldCap);
    ...
    vessel.Tick(0.001);
    vessel.Tick(20.001);  // 20 s — well past the ~9.5 s refill at 0.1466 mix-L/s

    Assert.IsTrue(fc.Manifold.Contents > 0.9 * SmallManifoldCap,
        $"Expected near-full manifold, got {fc.Manifold.Contents}");
  }
  ...
  public void Manifold_StarvationStopsProduction() {
    ...
    var fc = MakeFuelCell(isActive: true, manifold: 0);
  ...
  public void ValidUntil_ProductionProjectsToManifoldEmpty() {
    ...
    // Expected: manifold ≈ 1.466 mix-L, drain ≈ Activity·(Lh2Rate+LoxRate).
    // With Activity ≈ 1, dt ≈ 1.466 / 7.27e-4 ≈ 2017 s.
    Assert.IsTrue(fc.ValidUntilSeconds > 1500 && fc.ValidUntilSeconds < 2500,
        $"Expected ~2017 s manifold-empty horizon, got {fc.ValidUntilSeconds}");
  }
