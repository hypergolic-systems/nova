...
public static class SystemTags {
  ...
  public static List<string> For(IEnumerable<VirtualComponent> components) {
    ...
    foreach (var c in components) {
      // Battery, Command, FuelCell, SolarPanel — moved to nova-sim
      // (Rust). Their UI tags now live with the Rust mirror state;
      // C# only tags components still owned by VirtualVessel.
      switch (c) {
        case Light _:
