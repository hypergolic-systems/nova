...
public class NovaVesselModule : VesselModule {
  ...
  /// <summary>
  /// Rust simulator handle for this vessel. Holds typed pointers
  /// into the per-vessel mirror arena. Built when Virtual is built
  /// (same trigger points: first launch, save load, dock/undock).
  /// Null until <see cref="Virtual"/> is also non-null.
  /// </summary>
  ...
  // Retained as a no-op for back-compat with InvalidateSolarData()
  // call sites. Solar lives in nova-sim now; this flag does nothing.
...
#pragma warning restore CS0414
  ...
  /// Also rebuilds the Rust-side mirror handle for FFI-owned
  /// components (Battery, Command).
  /// </summary>
  ...
  private void RegisterFfiHandle() {
    var addon = NovaWorldAddon.Instance;
    if (addon == null || vessel == null) return;
    Handle?.Dispose();
    double ut;
    try { ut = Planetarium.GetUniversalTime(); } catch { ut = 0; }
    Handle = addon.RegisterVessel(vessel, ut);
  }
  ...
  public void FixedUpdate() {
    ...
    if (Virtual == null) return;
    // Solar moved to nova-sim — `solarDataDirty` no longer drives a
    // C# recompute, but we leave the flag in place so part modules
    // that mutate panel deploy state keep their existing call sites.
    // Re-purpose later when SolarPanel ports to FFI.
    ...
    Virtual.Tick(ut);
    NovaWorldAddon.Instance?.Tick(ut);
