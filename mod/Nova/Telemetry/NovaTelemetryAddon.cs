  ...
  private IEnumerator AttachWhenHostReady() {
    ...
    for (int attempt = 0; attempt < MaxAttempts; attempt++) {
      ...
      if (host != null && TopicRegistry.Instance != null) {
        ...
        if (host.GetComponent<NovaScienceArchiveTopic>() == null) {
          host.AddComponent<NovaScienceArchiveTopic>();
          NovaLog.Log("NovaScienceArchive topic attached to Dragonglass host");
        }
        if (host.GetComponent<NovaTimewarpTopic>() == null) {
        ...
        if (host.GetComponent<NovaPartInfoTopic>() == null) {
          host.AddComponent<NovaPartInfoTopic>();
          NovaLog.Log("NovaPartInfo topic attached to Dragonglass host");
        }
        yield break;
      }
      yield return null;
