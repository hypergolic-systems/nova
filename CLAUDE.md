...
- **Persistence — proto, not ConfigNode. No `[KSPField]`. No PartModule-owned runtime state.** Nova writes its own `.nvs` (save) and `.nvc` (craft) files defined by `proto/nova.proto`; stock KSP's `ConfigNode` save tree is bypassed end-to-end. `[KSPField(isPersistant = true)]` on a `NovaXxxModule` field writes into a tree Nova never reads — it's dead weight. A plain transient field on the module (a `private bool` with no attribute) is just as bad: it gives the illusion of state but vanishes on quicksave/reload or vessel unload, silently breaking anything that depended on the value across ticks. **Mutable runtime state lives on the `VirtualComponent`** and persists via `Save(PartState)` / `Load(PartState)` into a per-component `XxxState` proto message (`Nova.Core/Components/Thermal/Radiator.cs` is the canonical example). The PartModule is a thin adapter that reads/writes through `Components` — it owns no state of its own. The only legitimate `isPersistant = true` is the narrow case where KSP *itself* still reads the value (e.g. an animation flag stock code paths reference).
